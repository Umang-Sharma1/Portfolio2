export const typeDefs = `#graphql
  ...
  type Query {
    ...
    contactMessages(
      status: MessageStatus
      pagination: PaginationInput
    ): [ContactMessage!]! @auth(requires: ADMIN)
    
    # Get pending messages
    pendingMessages: [ContactMessage!]! @auth(requires: ADMIN)
    
    # Get spam statistics
    spamStats: SpamStats! @auth(requires: ADMIN)
  ...
  type Mutation {
    ...
    # Project mutations
    createProject(input: UpdateProjectInput!): Project! @auth(requires: ADMIN)
    updateProject(id: ID!, input: UpdateProjectInput!): Project! @auth(requires: ADMIN)
    deleteProject(id: ID!): MutationResponse! @auth(requires: ADMIN)
    
    # Skill mutations
    createSkill(input: UpdateSkillInput!): Skill! @auth(requires: ADMIN)
    updateSkill(id: ID!, input: UpdateSkillInput!): Skill! @auth(requires: ADMIN)
    deleteSkill(id: ID!): MutationResponse! @auth(requires: ADMIN)
    
    # Update skill project count (sync with projects)
    syncSkillProjectCounts: MutationResponse! @auth(requires: ADMIN)
    ...
    markMessageAsSpam(id: ID!): ContactMessage! @auth(requires: ADMIN)
    deleteMessage(id: ID!): MutationResponse! @auth(requires: ADMIN)
    
    # Leaderboard mutations
    verifyScore(id: ID!): LeaderboardEntry! @auth(requires: ADMIN)
    deleteScore(id: ID!): MutationResponse! @auth(requires: ADMIN)
    
    # Analytics mutations
    generateAnalytics(periodType: PeriodType!): Analytics! @auth(requires: ADMIN)
  }
  ...
  extend type Query {
    ...
    currentUser: AdminUser @auth(requires: ADMIN)
  }
  ...
  extend type Mutation {
    ...
    adminLogoutAll: Boolean! @auth(requires: ADMIN)
    
    # Refresh access token (uses httpOnly cookie)
    refreshToken: AuthResponse!
    
    # Get current user (alternative to currentUser query)
    me: AdminUser
  }
